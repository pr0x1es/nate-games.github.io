...
var EngineLoader = {
  ...
  loadAndInstantiateWasmAsync: function (
    src,
    fromProgress,
    toProgress,
    callback
  ...
  setupWasmStreamAsync: async function (src, fromProgress, toProgress) {
    ...
    Module.instantiateWasm = function (imports, successCallback) {
      WebAssembly.instantiateStreaming(fetchFn(src), imports)
        .then(function (output) {
  ...
  loadWasmAsync: function (exeName) {
    if (
      EngineLoader.stream_wasm &&
      typeof WebAssembly.instantiateStreaming === "function"
    ) {
      EngineLoader.setupWasmStreamAsync(exeName + ".wasm", 10, 50);
      EngineLoader.loadAndRunScriptAsync(
        exeName + "_wasm.js",
        EngineLoader.wasmjs_size,
        0,
        10
      );
    } else {
      EngineLoader.loadAndInstantiateWasmAsync(
        exeName + ".wasm",
        0,
        40,
        function () {
  ...
  loadAndRunScriptAsync: function (
    src,
    estimatedSize,
    fromProgress,
    toProgress
  ...
  load: function (appCanvasId, exeName) {
    ...
    if (Module["isWASMSupported"]) {
      EngineLoader.loadWasmAsync("assets/" + exeName);
    } else {
  ...
  addProgress: function (canvas) {
    ...
    canvas.insertAdjacentHTML(
      ...
      '<div id="' +
        ...
        '" class="canvas-app-progress"><div id="' +
        Progress.bar_id +
        '" class="canvas-app-progress-bar" style="width: 0%;"></div></div>'
    );
...
var Module = {
  ...
  prepareErrorObject: function (err, url, line, column, errObj) {
    ...
    var error =
      errObj ||
      (typeof window.event != "undefined" ? window.event.error : "") ||
      err ||
      "Undefined Error";
    var message = "";
    var stack = "";
    ...
    if (Module.hasWebGLSupport()) {
      ...
      if (typeof params["can_not_download_file_callback"] === "function") {
        GameArchiveLoader.addFileDownloadErrorListener(
          params["can_not_download_file_callback"]
        );
      }
      ...
      GameArchiveLoader.setFileLocationFilter(
        params["archive_location_filter"]
      );
...
window.onerror = function (err, url, line, column, errObj) {
  if (typeof Module.ccall !== "undefined") {
    ...
    Module.ccall(
      ...
      "null",
      ["string"],
      [JSON.stringify(errorObject.stack)]
    );
  }
